...
```{r}
clf_LR <- readRDS(file = "../models/LogisticRegressionClassifier_Full.Rds")
...
```{r}
predict(clf_LR, type = "response", newdata = woman5)
```

And in fact we check and woman5 does have diabetes.

```{r}
predict(clf_LR, type = "response", newdata = woman18)
```

And once again our model was correct, woman18 does not have diabetes.

```{r}
predict(clf_LR, type = "response", newdata = woman37)
```
